...
## share.html — making files to post

A full page embed that turns any work in `img/` into a file sized for posting.
Same rules as `sovrn-bot/tools/share.py`, and the two agree: a Latent Couture
still is 825 x 1200 from either. Bare artwork, native aspect ratio, nothing
overlaid and nothing padded; where a shape falls outside what X shows whole it
**says so** rather than altering the work, and the trim option is opt-in and cuts
from the top.

Three things constrain it:

- **Artwork must be read from the same origin.** Canvas export (`toBlob`) throws
  on a tainted canvas, and pulling an image from raster.art taints it. Reading
  from this repo over GitHub Pages is what makes the whole thing work.
- **byteGANs need the embedded GIF, not the SVG.** Rasterising the wrapper and
  scaling that up interpolates twice, neither step a whole number, and the pixel
  grid smears — and JPEG then rings on every hard edge. One came out with **142
  distinct colours**. Pulling the GIF out, scaling by a whole-number factor with
  `imageSmoothingEnabled = false`, and writing **PNG** gives 4 colours and a
  clean 1199 x 1199 (11 x 109).
- **Animated works export one frame.** Real motion needs a GIF encoder, which is
  not worth carrying in a page; the CLI does that.

`share-manifest.json` is generated by `sovrn-bot/tools/build-share-manifest.py`
and takes titles and artists from the bot's own catalogue, so the page cannot
drift from it. Regenerate after adding artwork.

Remember this page needs the `< >` **full page embed** type like every other
page here, and any link it grows needs `target="_blank"`.
